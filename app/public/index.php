...
switch (true) {
    ...
    case preg_match('#^/api/devices/(\d+)$#', $path, $m) && $method === 'PATCH':
        (new ApiController($portModel, $deviceModel))->updateDevice((int) $m[1]);
        break;
    ...
    case preg_match('#^/devices/(\d+)/ips/(\d+)$#', $path, $m) && $method === 'PATCH':
        (new DeviceController($deviceModel, $portModel))->updateIp((int) $m[1], (int) $m[2]);
        break;
